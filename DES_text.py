...
def encrypt(messageFile,key,outFile):
    ...
    while (bv.more_to_read):
        ...
        if bitvec.length() > 0:
            ...
            for i in range(16):
                RE_deep = RE.deep_copy()
                round_key = round_keys[i]
                newRE = RE.permute(expansion_permutation)
                out_xor = newRE ^ round_key 
                sbox_output = substitute(out_xor)
                pbox_output = sbox_output.permute(pbox_permutation)
                RE = LE ^ pbox_output
                LE = RE_deep
                

        final_string = RE.get_bitvector_in_hex() + LE.get_bitvector_in_hex()
        outFile.write(final_string)

def decrypt(message,key,outFile):
    round_keys= generate_round_keys( key )
    round_keys.reverse()
    bv = BitVector(hexstring = message)
   
    
    for i in range(bv.length()// 64):
        bitvec = bv[i*64:i*64+64]
        ...
        final_string = RE.get_bitvector_in_ascii() + LE.get_bitvector_in_ascii()
        outFile.write(final_string)
    return
